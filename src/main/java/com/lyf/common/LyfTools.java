...
public class LyfTools {
    ...
    public static String postJson(JSONObject jo, Map<String, String> headerMap, String url) throws IOException {
        OkHttpClient client = new OkHttpClient.Builder().connectTimeout(2, TimeUnit.SECONDS).writeTimeout(2, TimeUnit.SECONDS)
     ...
     * @return Map(fileName, size)  返回文件信息
     */
    public static Map multipartUpload2(MultipartFile[] files, HttpServletRequest request, String dir) {
        Map<String, String> fileInfoMap = new HashMap();
        if (LyfTools.arrEmpty(files)) {
            return null;
        }
        for (MultipartFile file : files) {
            ...
            try {
                ...
                //spring boot的上传地址--开发环境
                path = ResourceUtils.getURL("classpath:META-INF/resources").getPath() + dir;

                //spring boot的上传地址--linux生产环境--start
...
//                path = grandpaPath + dir;
//                System.out.println(path);

                //spring boot的上传地址--linux生产环境--end
    ...
    public static Properties readProps(String filePath) {
        ...
        InputStream in = in = LyfTools.class.getResourceAsStream(filePath);
        try {
            props.load(in);
        } catch (IOException e) {
            e.printStackTrace();
